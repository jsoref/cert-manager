...
`
	...
	It("Should error if a certificate is supplied with another name containing an invalid oid value", func() {
	...
	It("Should error if a certificate is supplied with another name without a UTF8 value", func() {
		_, err := createCertificate(f, []cmapi.OtherName{
			{
				OID: "1.3.6.1.4.1.311.20.2.3",
			},
